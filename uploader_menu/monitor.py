...
from uploader_core import database, network
from uploader_menu import display
...
def system_usage():
    ...
    uploader_monitor = PrettyTable()
    uploader_monitor.field_names = [f"{display.cyan_color}Σ Upload{display.cornsilk_color}",
    ...
    while True:
        ...
        if database.get_parameter('in_submenu') is False:
            ...
            uploader_monitor.del_row(0)
            print(uploader_monitor)
            display.line_jumper(+11)
