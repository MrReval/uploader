from uploader_core import database
from uploader_menu import udp_submenu, display, network_submenu
from uploader_core.network import get_size
from os import system, path


def reload_uploader_service():
    if path.isfile('/var/www/uploader/range_ips'):
        database.set_parameter('range_ips', open('/var/www/uploader/range_ips').read())
    else:
        system('cp /var/www/uploader/else/range_ips /var/www/uploader/range_ips')
        database.set_parameter('range_ips', open('/var/www/uploader/range_ips').read())
    system('systemctl restart uploader.service')
