from uploader_core.time import get_now_date, get_now_time
from uploader_core.network import get_size


def store_restart_uploader_uploader_log():
    file = open('/var/www/uploader/uploader.log', 'a')
    file.write(f"date: {get_now_date()} - "
               f"time: {get_now_time()} - "
               f"uploader.service restarted\n")
    file.close()
